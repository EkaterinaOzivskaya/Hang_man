...
def get_word(arg):
    return random.choice(arg).upper()

def display_hangman(tries):    # tries - кол.попыток
...
def play(word):
    ...
    for _ in range(6):
        ...
        if _ == 0 and guess == False:
            guessed = input(f'Введите слово или букву. У вас ровно {tries} попыток.. ')
        elif _ < 6 and _ != 0 and guess == False:
            guessed = input(f'Попробуйте ещё. У вас ровно {tries} попыток.. ')
        elif guess == True:
            guessed = input(f'Продолжайте в том же духе. У вас {tries} попыток.. ')
        elif _ == 6 and guess == False:
            guessed = input('Последняя возможность ')
        if guessed.isalpha() != True:
            print('Введите слово или букву кирилицей.. ')
        else:
            guessed = guessed.upper()
            if len(guessed) == 1:
                if guessed in word:
                    a = word.count(guessed)
                    for i in range(a):
                        word_completion = word_completion[:word.find(guessed)] + list(guessed) + word_completion[word.find(guessed) + 1:]
                        a = word[word.find(guessed) + 1:]
                    guess == True
                    print()
            ...
            if guessed == word:
                ...
                print(word)
                answer = input('Хотите попробовать снова? (д/н).. ')
                if answer == 'д':
                    print()
                    ...
                    break
            if len(guessed) > 1 and guessed != word:
                tries -= 1
                print('Неправильно')
